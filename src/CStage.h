...
public:
		...
		vec(0.1f)
	{
		...
		board.addRequest({ 0,"漂う魚影", "深海に潜む影を討伐せよ" , 5, CLoad::Instance().getEfaceGrh(ZPTHOMMOG)});
		board.addRequest({ 1,"奈落の底のモノ", "深海に巣くう脅威を取り払え", 15, CLoad::Instance().getEfaceGrh(YTHOGTHA) });
		board.addRequest({ 2,"深淵の恐怖", "深淵に住まう王を討伐せよ", 22, CLoad::Instance().getEfaceGrh(CTHULHU) });
